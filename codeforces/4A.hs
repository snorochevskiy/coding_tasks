-- Watermelon
-- http://codeforces.com/problemset/problem/4/A?locale=en
...
main :: IO()
main = do
  watermelonWeight <- readLn :: IO Int
  let result = if (watermelonWeight >= 4) && (watermelonWeight `mod` 2 == 0) then "YES" else "NO"
  putStr result
